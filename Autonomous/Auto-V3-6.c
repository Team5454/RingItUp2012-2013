...
//Sensor, S3,     colorSensorVal, sensorI2CHiTechnicColor
...
const tMUXSensor IR = msensor_S3_1;
const tMUXSensor colorSensorVal = msensor_S3_2;
...
int WAMleftServoStart = 127;	// WAM Left Servo Start Point
...
void selectAutoActions()
{
	...
	while(true)
	{
		...
		// Keep AutoActions within a range of valid values
		if (AutoActions > 7) AutoActions = 7;
		...
		case 6: nxtDisplayCenteredTextLine(6, "Center & LineUp");break;
		case 7: nxtDisplayCenteredTextLine(6, "Three Rings");break;
		default: nxtDisplayCenteredTextLine(6, "Error");break;
		}
		//---------------------------------------------------------------
		if(nNxtButtonPressed == 3)	// NXT Orange Button
...
int selectLocation(int value)
{
	...
	if(value==6)	// Height to allow the robot to drive around with up to two rings in the crown
	{
		pegHeightTicks = inchesToTicks(threeRingHeight);
		//nxtDisplayString(6, "btn 9 pressed");
	}
...
///////////////////////////////////////////////////////////////////////
	...
	{
	case 7:
		elevatorHeightTicks = selectLocation(5);	// Determine the number of encoder ticks based on the commanded elevator height
		elevatorGoToHeightFailSafe(elevatorHeightTicks, 2000);	// Raise elevator to the commanded height to score a ring

		// put servo up in front of bot
		servo[RingServo] = 0;
	}
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	case 1:	// Score Ring on the column designated by the IR Beacon
		...
		irSensorVal = HTIRS2readACDir(IR);
		...
		{
		case 1:
			move_forward(12, 2000, 90, 90);
			wait10Msec(100);
			turngyro_left(90.0, 90);
			wait10Msec(100);
			move_forward(26, 4000, 80, 80);
			wait10Msec(100);
			turngyro_right(45.0, 90);
			motor[RAMright] = 95;
			motor[RAMleft] = 95;
			wait10Msec(100);
			move_forward(20, 3000, 90, 90);
			wait10Msec(100);
			motor[Right1] = 80;
			motor[Right2] = 80;
			motor[Left1] = -80;
		...
		case 3:
			move_forward(40, 2000, 90, 90);
			wait10Msec(100);
			turngyro_left(45.0, 90);
			motor[RAMright] = 95;
			motor[RAMleft] = 95;
			wait10Msec(100);
			move_forward(30, 1000, 90, 90);
			wait10Msec(100);
			motor[Right1] = -50;
			motor[Right2] = -50;
			motor[Left1] = 90;
	...
	case 6:	// Score on the center column & line up with the dispenser
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//Case: 7 || Grab and Score Three Rings=========================================================================//
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	case 7:	// Three Rings
		...
		wait1Msec(delayAutoStartValue);
		elevatorHeightTicks = selectLocation(6);	// Determine the number of encoder ticks to raise the elevator to the drive height
		elevatorGoToHeight(elevatorHeightTicks);	// Raise elevator to the commanded height
		wait1Msec(500);

		servo[WAMleftServo] = 20;
		servo[WAMrightServo] = 235;
		wait10Msec(500);

		move_forward(2.5, 1500, 30, 30);
		wait10Msec(500);

		servo[WAMleftServo] = WAMleftServoStart;
		servo[WAMrightServo] = WAMrightServoStart;
		wait10Msec(50);

		// go down til you hit ring limit
		ClearTimer(T1);
		while(time1[T1] < 1500 && ringLimit == 0)
		{
			motor[elevatorA] = -60;
			motor[elevatorB] = -60;
		}
		...
		move_forward(20, 2000, 60, 60);
		wait1Msec(2000);

		ClearTimer(T1);
		HTCS2readRGB(colorSensorVal, red, green, blue);
		while((time1[T1]<4000) && (red <= 100))
		{
			HTCS2readRGB(colorSensorVal, red, green, blue);
			motor[Left1] = 50;
			motor[Left2] = 50;
		...
		// Score bottom ring
		elevatorHeightTicks = selectLocation(1);	// Determine the number of encoder ticks based on the commanded elevator height
		elevatorGoToHeight(elevatorHeightTicks);	// Raise elevator to the commanded height to score a ring
		wait10Msec(100);
		elevatorHeightTicks = selectLocation(4);	// Determine the number of encoder ticks to lower the elevator and score the ring
		elevatorGoToHeight(elevatorHeightTicks);	// Score Ring
		...
		// Score middle ring
		elevatorHeightTicks = selectLocation(2);	// Determine the number of encoder ticks based on the commanded elevator height
		elevatorGoToHeight(elevatorHeightTicks);	// Raise elevator to the commanded height to score a ring
		wait10Msec(100);
		elevatorHeightTicks = selectLocation(4);	// Determine the number of encoder ticks to lower the elevator and score the ring
		elevatorGoToHeight(elevatorHeightTicks);

		// Score top ring
		elevatorHeightTicks = selectLocation(3);	// Determine the number of encoder ticks based on the commanded elevator height
		elevatorGoToHeight(elevatorHeightTicks);	// Raise elevator to the commanded height to score a ring
		wait10Msec(100);
		elevatorHeightTicks = selectLocation(4);	// Determine the number of encoder ticks to lower the elevator and score the ring
		elevatorGoToHeight(elevatorHeightTicks);
