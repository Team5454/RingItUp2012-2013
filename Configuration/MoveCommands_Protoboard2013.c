...
void nmf(float dist, int tstop, int lspeed, int rspeed)
{
  ...
  {
    encdiff = nMotorEncoder[RightMotor] - nMotorEncoder[LeftMotor];
    // if rightencoder > leftencoder
    // slow down RightMotor
    if (encdiff > 20)
    {
      motor[RightMotor] = rspeed - 20;
    }
    else if (encdiff > 10)
    {
      motor[RightMotor] = rspeed - 10;
    }
    // if rightencoder > leftencoder
    // slow down leftmotor
    else if (encdiff < -20)
    {
      motor[LeftMotor] = lspeed - 20;
    }
    else if (encdiff < -10)
    {
      motor[LeftMotor] = lspeed - 10;
    }
    else
    {
      motor[LeftMotor] = lspeed;
